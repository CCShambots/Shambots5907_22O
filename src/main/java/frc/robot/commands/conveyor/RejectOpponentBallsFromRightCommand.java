...
import static frc.robot.util.ballTracker.Ball.BallColorType.Opposing;

public class RejectOpponentBallsFromRightCommand extends CommandBase {
    private Conveyor conveyor;
    private BallTracker tracker;

    public RejectOpponentBallsFromRightCommand(Conveyor conveyor, BallTracker ballTracker) {
    ...
    public void end(boolean interrupted) {
        conveyor.stopLeftConveyor();

        if(conveyor.shouldEndIntakeSequence()) {
            conveyor.goToStateCommandDelayed(Idle).schedule();
        } else {
            if(tracker.getNumberOfBalls() > 1) {
                conveyor.goToStateCommandDelayed(WaitForBallFromRight).schedule();
            } else {
                conveyor.goToStateCommandDelayed(IntakeRight).schedule();
            }
        }
    }
}
