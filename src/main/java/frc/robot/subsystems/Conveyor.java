...
public class Conveyor extends StatedSubsystem<Conveyor.ConveyorState> {
    ...
    public Conveyor() {
        ...
        setContinuousCommand(WaitForBallFromLeft, new FunctionalCommand(() -> {}, () -> {},
                (interrupted) -> {if(!interrupted) requestTransition(Idle);}, () -> ballTracker.isBallAtPos(Center)));

        addTransition(WaitForBallFromLeft, Idle, new InstantCommand(this::stopAll));

        setContinuousCommand(IntakeLeft, new FunctionalCommand(() ->{}, () -> {}, (interrupted) -> {if(shouldEndIntakeSequence && !interrupted) requestTransition(Idle);}, () -> shouldEndIntakeSequence));
        ...
        setContinuousCommand(WaitForBallFromRight, new FunctionalCommand(() -> {}, () -> {}, (interrupted) ->
                { if(!interrupted) requestTransition(Idle);}, () -> ballTracker.isBallAtPos(Center)));

        addTransition(WaitForBallFromRight, Idle, new InstantCommand(this::stopAll));

        setContinuousCommand(IntakeRight, new FunctionalCommand(() ->{}, () -> {}, (interrupted) -> {if(shouldEndIntakeSequence && !interrupted) requestTransition(Idle);}, () -> shouldEndIntakeSequence));
        ...
        //We want to be able to start shooting from almost any state
        addMultiTransitions(StartShooting,
                new InstantCommand(() -> {intakeLeftConveyor(); intakeRightConveyor(); shootTimer.reset(); shootTimer.start();}),
                Idle
                , ShuffleToRight, ShuffleToLeft, StartIntakeLeft, StartIntakeRight, IntakeLeft, IntakeRight, WaitForBallFromLeft, WaitForBallFromRight
            );


        setContinuousCommand(StartShooting, evaluateShootCommand());
        ...
        setContinuousCommand(ShootFromCenter, new ParallelRaceGroup(
                ...
                new FunctionalCommand(() -> {}, () -> {}, (interrupted) -> {}, () -> shootTimer.get() > MAX_SHOOT_TIME))
    ...
    private Command evaluateShootCommand() {
    ...
    public Command resetTrackerForAuto() {
        return new InstantCommand(() -> resetBallTracker(List.of(
            new Ball(BallColorType.Ours, leftProxSensor.isActivated() ? Left : PastLeft),
